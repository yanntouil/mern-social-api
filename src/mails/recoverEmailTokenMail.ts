...
<p>
    You as ask a new email validation on our website, If that's not you, disregard this message. Otherwise, click on the button below.
</p>
...
Lorem ipsum, dolor sit amet consectetur adipisicing elit. Rem magnam labore voluptatum quasi sequi ullam tempora placeat id ducimus, voluptate perferendis et, expedita qui provident distinctio, molestiae doloremque. Numquam, maiores.
`
export default async function recoverEmailTokenMail({username, email, link}: { username: string, email: string, link: string }) {
    return mailService.sendMail({
        from: config.get<string>('mailFrom'),
        to: email,
        subject: `New email validation`,
