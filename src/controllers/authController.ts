...
export default {
     ...
     */
    registrer : async (req: Request<{}, {}, AuthRegistrerBodyInterface>, res: Response) => {
     ...
     */
    login : async (req: Request<{}, {}, AuthLoginBodyInterface>, res: Response) => {
     ...
     */
    update : async (req: Request<{}, {}, AuthUpdateBodyInterface>, res: Response) => {
        ...
        if (username && user.username.toString() !== username && await User.findOne({ username })) 
     ...
     */
    recoverEmailToken : async (req: Request<{}, {}, AuthRecoverEmailTokenBodyInterface>, res: Response) => {
     ...
     */
    recoverPassword : async (req: Request<{}, {}, AuthRecoverPasswordBodyInterface>, res: Response) => {
     ...
     */
    authenticationToken : async (req: Request, res: Response) => {
    ...
    follow : async (req: Request, res: Response) => {
        ...
        if (session.user.id.toString() === id) throw new RequestError(`Can not follow himself`, 400)
        const following = await User.findByIdAndUpdate(id, { $addToSet: { followers: session.user.id } })
        if (!following) throw new RequestError(`User not found`, 400)
        await User.findByIdAndUpdate(session.user.id, { $addToSet: { followings: following.id } })
    ...
    unfollow : async (req: Request, res: Response) => {
        const { id } = req.params, { session } = res.locals
        if (!objectIdIsValid(id)) throw new RequestError(`Invalid ID`, 400)// Move in validation
        const following = await User.findByIdAndUpdate(id, { $unset: { followers: session.user.id } })
        if (!following) throw new RequestError(`User not found`, 400)
        await User.findByIdAndUpdate(session.user.id, { $unset: { followings: following.id } })
 ...
 */
interface AuthRegistrerBodyInterface {
    username: string
    email: string
    password: string
    redirect: string
}
interface AuthLoginBodyInterface {
    username: string
    email: string
    password: string
}
interface AuthUpdateBodyInterface {
    username: string
    password: string
    email: string
    about: string
...
interface AuthRecoverEmailTokenBodyInterface {
    username?: string
    password: string
    email?: string
    redirect: string
}
interface AuthRecoverPasswordBodyInterface {
    email: string
    redirect: string
}